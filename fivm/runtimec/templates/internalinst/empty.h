...
/*
 * This is an empty template for a Fiji VM internal instrumentation header
 * file.  You can copy and edit this template, and then supply a path to
 * the file using the --internal-inst option.  This will allow you to
 * insert extensive instrumentation into the guts of the Fiji VM.  For the
 * purposes of performance and robustness, internal instrumentation files
 * must follow rigid guidelines.  Therefore, we highly recommend that you
 * use this template and edit it only using the guidance in the comments
 * that follow.
 *
 * General conventions:
 * - The code in the FIVMR_INTINST_HEADER block becomes part of the Fiji VM
 *   headers, included by both Fiji VM's runtime code and all generated C
 *   code (i.e. Java code).
 * - The code in the FIVMR_INTINST_BODY block becomes its own code module,
 *   compiled with Fiji VM compiler options, and linked to the resulting
 *   executable.
 * - All fields and local variables should start with 'ii_'.  For example,
 *   ii_foo instead of just foo.
 * - All global variables, types, and functions should start with 'fivmr_ii_'.
 *   For example, fivmr_ii_foo instead of just foo.
 *
 ...
 *
 * There are also other example files in this directory, which follow a
 * similar pattern, but show how to use the various macros.
 */
