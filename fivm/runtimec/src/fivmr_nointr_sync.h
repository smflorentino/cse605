...
/*
 * Synchronization support for platforms that have the notion of disabling interrupts.
 */
...
static inline void fivmr_cas_void(uintptr_t *ptr,
				  uintptr_t comparand,
				  uintptr_t newValue) {
    fivmr_interrupt_level cookie;
    fivmr_interrupt_disable(cookie);
    if (*ptr == comparand) {
	*ptr = newValue;
    }
    fivmr_interrupt_enable(cookie);
}
...
/* FIXME: implement DCAS ... though nobody uses it right now */
...
static inline void fivmr_fence(void) {
    fivmr_compilerFence(); /* we don't need hardware fences, since we're uniprocessor */
}
