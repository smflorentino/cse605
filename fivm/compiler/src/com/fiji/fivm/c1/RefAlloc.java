...
public class RefAlloc {
    RefsLiveAtSafe rlas;
    HashMap< Var, Integer > map=new HashMap< Var, Integer >();
    int numRefs;
    
    public RefAlloc(Code c) {
	rlas=c.getRefsLiveAtSafe();
	...
	numRefs=gra.getNumRegs();
	for (Var v : gra.relevantVars()) {
	    map.put(v,gra.getColor(v));
	}
        
        if (Global.verbosity>=3) {
            Global.log.println("RefAlloc results: "+map);
        }
    }
    ...
    public boolean isRelevantVar(Var v) {
	return map.containsKey(v);
    }
    
    public boolean isRelevantForAssign(Var v) {
        return rlas.isRelevantForAssign(v);
    }
    ...
    public int varAssignment(Var v) {
	return map.get(v);
    }
}
